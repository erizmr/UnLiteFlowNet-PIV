...
def train(model, optimizer, criterion, data_loader):
    ...
    for x, y in data_loader:
        ...
        print("Finished this epoch %1.3f %%, real time EPE loss %1.3f, time used(seconds) %1.3f, expected time to finish %1.3f"
              % ( percent, real_timeEPE, total_time, (100 - percent)*total_time/percent))
        
    return train_loss/len(data_loader.dataset), flow2_EPE/len(data_loader.dataset)
...
def train_model(model,
  ...
  validation_loader = DataLoader(validate_dataset, batch_size=test_batch_size, shuffle=False, num_workers=4, pin_memory=True)
  ...
  for epoch in range(epoch_trained, n_epochs):
      ...
      print("Memory allocated: ", torch.cuda.memory_allocated() / 1024 /1024 /1024)

      GPUtil.showUtilization()

      logs = {}
      train_loss, train_loss_epe = train(model, optimizer, criterion_train, train_loader)
      validation_loss_epe = validate(model, criterion_validate, validation_loader)
      ...
      print("Epoch: ",epoch,", Avg. Train EPE Loss: %1.3f" % train_loss_epe, 
            " Avg. Validation EPE Loss: %1.3f" % validation_loss_epe,
            "Time used this epoch (seconds): %1.3f" % (end_time - start_time),
            "Time remain(hrs) %1.3f" % (total_time / (epoch+1) *(n_epochs-epoch) / 3600 ))
      ...
      if (epoch + 1) % 5 == 0:
        ...
        # Do the save
        save_model(model, optimizer, train_loss, para_dict,"UnLiteFlowNet_checkpoint_%d_"% epoch)
  
  test_loss_epe = validate(model, criterion_validate, test_loader)    
  print(" Avg. Test EPE Loss: %1.3f" % test_loss_epe, "Total time used(seconds): %1.3f" % total_time)
  print("")
  ...
  para_dict['time used(seconds)'] = total_time
  para_dict['test EPE'] = test_loss_epe
  save_model(model, optimizer, train_loss, para_dict,"UnLiteFlowNet_%d_"% epoch)
