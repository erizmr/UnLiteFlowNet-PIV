		...
		class Features(torch.nn.Module):
			def __init__(self):
				...
				self.moduleOne = torch.nn.Sequential(
					torch.nn.Conv2d(in_channels=1, out_channels=32, kernel_size=7, stride=1, padding=3),
...
def estimate(tensorFirst, tensorSecond, model, train = False):
	...
	tensorFlow = torch.nn.functional.interpolate(input=raw_output[-1], size=(intHeight, intWidth), mode='bilinear', align_corners=False)
