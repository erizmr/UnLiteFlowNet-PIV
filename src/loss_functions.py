...
def multiscaleUnsupervisorError(tensorFlowForward, tensorFlowBackward, tensorFirst, tensorSecond, weights=None):

    h, w = tensorFirst.shape[-2], tensorFirst.shape[-1]

    def one_scale(tensorFirst, tensorSecond, tensorFlowForward, tensorFlowBackward):
        lambda_s = 3.0
        lambda_c = 0.2
    ...
    for i, weight in enumerate(weights):
        ...
        scale_factor = 1/(2**i)
        loss += weight * one_scale(tensorFirst, tensorSecond, tensorFlowForward[-1-i] * scale_factor, tensorFlowBackward[-1-i] * scale_factor)
        h = h // 2
        w = w // 2
        tensorFirst = F.interpolate(tensorFirst, (h, w), mode='bilinear', align_corners=False)
        tensorSecond = F.interpolate(tensorSecond, (h, w), mode='bilinear', align_corners=False)
...
def charbonnierLoss(x, alpha=0.45, beta=1.0, epsilon=0.001):
    ...
    normalization = torch.tensor(batch * height * width * channels, requires_grad=False)

    error = torch.pow((x * torch.tensor(beta)).pow(2)  + torch.tensor(epsilon).pow(2), alpha)
...
def warpLoss(tensorFirst, tensorSecond, tensorFlow):
  ...
  tensorDifference = tensorFirst - Backward(tensorInput=tensorSecond, tensorFlow=tensorFlow)
  return charbonnierLoss(tensorDifference, beta=255.0)

def bidirectionalDataLoss(tensorFirst, tensorSecond, tensorFlowForward, tensorFlowBackward):
  """Compute bidirectional photometric loss"""
  return warpLoss(tensorFirst, tensorSecond, tensorFlowForward) + warpLoss(tensorSecond, tensorFirst, tensorFlowBackward)

# Flow consistency loss
def consistencyLoss(tensorFlowForward, tensorFlowBackward):
  """ Differentiable Charbonnier penalty function"""
  tensorFlowDifference = tensorFlowForward + Backward(tensorInput=tensorFlowBackward, tensorFlow=tensorFlowForward)
  return charbonnierLoss(tensorFlowDifference)

def bidirectionalConsistencyLoss(tensorFlowForward, tensorFlowBackward):
  return consistencyLoss(tensorFlowForward, tensorFlowBackward) + consistencyLoss(tensorFlowBackward, tensorFlowForward)
...
def bidirectionalSmoothnessLoss(tensorFlowForward, tensorFlowBackward):
  ...
  return smoothnessLoss(tensorFlowForward) + smoothnessLoss(tensorFlowBackward)
...
def biSecondSmoothnessLoss(tensorFlowForward, tensorFlowBackward):
  ...
  return secondSmoothnessLoss(tensorFlowForward) + secondSmoothnessLoss(tensorFlowBackward)
